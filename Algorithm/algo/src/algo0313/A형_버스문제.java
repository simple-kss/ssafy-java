...
/*
 * 
 * BFS => 호출상태를 콜스택에 많이 쌓아야함, 메모리를 낭비
 * DFS : 메모리는 BFS에 비해서 절약되지만, 많은 중복호출 발생
 * 메모이제이션 : 호출된 결과를 저장해서, 한번 호출했던 것은 저장한 값으로 대체
 * 
 * 
 */

public class A형_버스문제 {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int t = 1; t <= tc; t++) {
			...
			memo = new int[n][n][3]; // 행, 열, mode, 계산한 경로의 갯수를 저장할 배열
			for(int i = 0; i < n; i++) {
				for(int j = 0; j < n; j++) {
					for(int k = 0; k < 3; k++) {
						memo[i][j][k] = -1;
	...
	public static int go(int r, int c, int mode) {
		...
		if (mode == 0) { // 가로 -> 가로, 대각
			if (r == n - 1 && c + 1 == n - 1) {
				...
				if (m[r][c + 2] == 0 && m[r + 1][c + 1] == 0 && m[r + 1][c + 2] == 0) { // 가로 -> 대각
					cnt += go(r, c + 1, 1);
				}
			}
