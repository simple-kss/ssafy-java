...
//		
	...
	public void search(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		List<Food> list = dao.searchName(name);
		req.setAttribute("list", list);
...
//		
//		dao.insert(new Food(isbn, title, catalogue, nation, publishDate, publisher, author, price, currency, description));
...
//		ArrayList<Food> list = dao.selectAll(); // 컨트롤러에게 공개 안하는 메소드
//		req.setAttribute("list", list);
//		
//		
...
//		Food b = dao.selectOne(isbn); // 컨트롤러에게 공개 안하는 메소드
...
//		this.BookList(req, res);
	...
	public void specific(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		String codeString = req.getParameter("id");
		
		int code = Integer.parseInt(codeString);
		
		Food it = dao.search(code);
		
		req.setAttribute("item", it);
		
		RequestDispatcher dispatcher = req.getRequestDispatcher("/view/specific.jsp");
		dispatcher.forward(req, res);
	}
}
